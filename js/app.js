...
$(document).ready(function () {
      ...
      $.ajax(settings).done(function (response) {
        ...
        for (var i = 0; i < response.length && i < limit; i++) {
          ...
          */
          ...
          <td>${response[i].email}</td>
          <td><a href='#' class='delete' data-id='${response[i]._id}'>Del</a></td><td><a href='#update-contact-container' class='update' data-id='${response[i]._id}' data-name='${response[i].name}' data-email='${response[i].email}'>Update</a></td></tr>`;
    ...
    function updateForm(id, contactname, contactemail) {
      ...
      var jsondata = { "name": contactname, "email": contactemail};
      var settings = {
        "async": true,
        "crossDomain": true,
